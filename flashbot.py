...
def yaml_load(file_path):
    with open(file_path, 'r') as fr:
        configs = yaml.load(fr, yaml.FullLoader)
...
class Sniper():
    ...
    def __init__(self, configs, args):
        ...
        self.pancake_route_address = address.pancake_route_address
        ...
        self.pancake_factory_address = address.pancake_factory_address
        ...
        self.pancake_pair_address = address.pancake_pair_address
        ...
        # WBNB Address
        self.wbnb_address = address.wbnb_address
        # WBNB ABI
        ...
        # BUSDT Address
        self.busdt_address = address.busdt_address
        # BUSDT ABI
        ...
        self.address_1 = address.wbnb_address
        # BNB/USDT
        self.token_1_check_sum_address = self.wbnb_check_sum_address
        # 交易结束地址: 新币地址, 需要用户输入
        self.address_2 = args.token_address
        # 新币
        self.token_2_check_sum_address = Web3.toChecksumAddress(self.address_2)
        # 合约实例
        self.token_instance = self.web3.eth.contract(address=self.token_2_check_sum_address, abi=self.wbnb_abi)
        
        # 交易对地址
        self.pair_address = self.pancake_factory_check_sum_instance.functions.getPair(self.token_1_check_sum_address, self.token_2_check_sum_address).call()
        # 流动性地址
        self.swap_pair_address = Web3.toChecksumAddress(self.pair_address)
        # 地址列表
        
        self.money_type = args.money_type
        ...
        # 滑点
        # 购买滑点百分比
        self.slippage_buy = args.buy_slip
        self.slippage_sell = args.sell_slip

        # 小数位数
        # BNB/USDT 小数位数
        self.decimal_1 = 18
        # 新货币小数位数
        self.decimal_2 = self.token_instance.functions.decimals().call()
        ...
        # 钱包信息
        # 钱包地址
        self.my_address = configs.wallet.address
        # 汇款地址
        self.my_check_sum_address = Web3.toChecksumAddress(self.my_address)
        # 钱包私钥
        self.my_private_key = configs.wallet.private
    ...
    def approve_token(self):
        # 代币合约实例化
        token_instance = self.token_instance
    ...
    def buy_market_bnb(self):
        # 购买
        amountOutMin = int((self.pool_token_count / self.pool_bnb_count * self.count_token_1) * ( 1 - self.slippage_buy))
        tx = self.pancake_route_check_sum_instance.functions.swapExactETHForTokens(amountOutMin, self.addresses_buy, self.my_check_sum_address, self.deadline).buildTransaction(
            {
                'chainId': self.chain_id,
                'nonce': self.web3.eth.getTransactionCount(self.my_check_sum_address),
    ...
    def sell_market(self, count_in_s):
        ...
        self.get_pool_token_count()

        amount_in = self.web3.toWei(count_in_s, self.decimal_change(decimal_int=self.decimal_2))
        amountOutMin = round((self.pool_bnb_count / self.pool_token_count * amount_in) * (1 - self.slippage_sell), 4)
        # 出售
        tx = self.pancake_route_check_sum_instance.functions.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount_in, amountOutMin, self.addresses_sell, self.my_check_sum_address, self.deadline).buildTransaction(
            {
                'chainId': self.chain_id,
                'nonce': self.web3.eth.getTransactionCount(self.my_check_sum_address),
    ...
    def get_pool_token_count(self):
        ...
        self.pool_bnb_count = reserve1
        self.pool_token_count = reserve2
        # 打印池子数据
    ...
    def getReserveOfToken(self):
        ...
        while self.liquid_flag == False:
            ...
            reserve1, reserve2 = self.get_pool_token_count()
    ...
    def print_all_para(self):
        print(f"[普通显示]钱包地址: {self.my_check_sum_address};")
        print(f"[普通显示]BNB/USDT: {self.money_type};")
        print(f"[普通显示]合约地址: {self.token_2_check_sum_address}; 新币精度: {self.decimal_2};")
        print(f"[普通显示]交易对地址: {self.pair_address};")
        print(f"[普通显示]截止时间: {self.deadline}; 买入滑点: {self.slippage_buy}; Buy Gas Price: {self.buy_gas_price}; Buy Gas Limit: {self.buy_gas_limit}; Sell Gas Price: {self.buy_gas_price}; Sell Gas Limit: {self.buy_gas_limit}; BNB/USDT_MIN: {self.token_1_min}; TOKEN_MIN: {self.token_2_min};")
        print(f"[普通显示]截止时间: {self.deadline}; 卖出滑点: {self.slippage_buy}; Buy Gas Price: {self.sell_gas_price}; Buy Gas Limit: {self.sell_gas_limit}; Sell Gas Price: {self.sell_gas_price}; Sell Gas Limit: {self.sell_gas_limit}; BNB/USDT_MIN: {self.token_1_min}; TOKEN_MIN: {self.token_2_min};")
        print(f"[普通显示]买入花费金额: {self.count_token_1};")
        print(f"[普通显示]买入单价: {self.act_price}; 消耗 BNB/USDT: {self.act_count_in}; 获取 Token: {self.act_count_out};")
        print(f"[普通显示]卖出百分比: {self.per_token_2};")
        print(f"[普通显示]止损百分比: {self.control_loss_per}; 止损价格: {self.control_loss_price};")
        print(f"[普通显示]止盈百分比: {self.control_prof_per}; 止盈价格: {self.control_prof_price};")
        # 返回
        return
...
def type_str2bool(v: str):
    '''Convert str-type args value to bool-type'''
    v = v.strip().lower()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--money_type', default='bnb', type=str)
    parser.add_argument('--robot', default=False, type=type_str2bool)
    parser.add_argument('--buy_amount', default=0.01, type=float)
    parser.add_argument('--profit_count', default=100, type=int)
    parser.add_argument('--loss_count', default=10, type=float)
    
    parser.add_argument('--only_buy', default=False, type=type_str2bool)
    parser.add_argument('--buy_price', default=0.0, type=float)
    parser.add_argument('--sell_price', default=0.0, type=float)
    parser.add_argument('--only_sell', default=False, type=type_str2bool)
    parser.add_argument('--sell_percent', '-sp', default=100, type=int)

    parser.add_argument('--buy_slip', default=10, type=int)
    parser.add_argument('--sell_slip', default=49, type=int)
    args = parser.parse_args()
    
    # 参数
    configs = yaml_load('./config.yaml')
    wallet = yaml_load('./wallet.yaml')
    configs.wallet = wallet
    sni = Sniper(configs, args)
    sni.count_token_1 = args.buy_amount
    sni.control_prof_per = args.profit_count
    sni.control_loss_per = args.loss_count
    sni.per_token_2 = args.sell_percent

    # 显示结果
    sni.print_all_para()
    sni.query_balance()
    sni.approve_to_pancake()
    ...
    # 策略2. 指定价格买入卖出。
    if args.only_buy:
        if args.buy_price != '0.0':
            sni.buy_model_type()
            sni.get_price()
        else:
            while True:
                if sni.self.real_time_price <= args.buy_price:
                    sni.buy_model_type()
            
        if args.sell_price != '0.0':
            while True:
                if sni.self.real_time_price >= args.sell_price:
                    sni.sell_model_type()
        # # 止盈模式开启
        sni.control_prof()
        # # 止损模式开启
